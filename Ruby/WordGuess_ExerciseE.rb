...
class Game
  ...
  def display_instructions
    ...
    puts %Q{
    At the start of each round of play, the game will randomly
    select a word that is no less than 5 and no more than 10 letters long.
    The game will challenge you to guess it. To make it easier to guess,
    the game will allow you to specify five consonants and one vowel.
    The correctly guessed consonants and vowels will appear, but the 
    remaining letters will be displayed as underscores "_" 
    This will help make it easier to see what letters remain so that you
    can guess the secret word.\n\n
    Good luck!\n\n\n\n\n\n\n\n\n
  ...
  def select_word
    ...
    words =  ["W I N D O W", "S T A T I O N", "H A M B U R G E R", 
				...
				"H A P P Y", "H O U S E", "C A R P E N T E R", "A N G R Y", 
				"J O Y O U S", "S E A R C H", "E N G I N E", "P R O G R A M", 
				"M O N O T O N O U S", "D I C T I O N A R Y", "T H E S A U R U S", 
				"P O S I T I V E", "C R E A T E", "D E S T R O Y", "S A V E D", 
				"B E G I N N I N G", "F U N E R A L", "D E A T H", "Q U I E T", 
				"M O M E N T S", "D I S C O V E R Y"]
  
    #Generate and return a random number between 0 and 19
    randomNo = rand(39)
  
    #Return a randomly selected word to the calling statement
    return words[randomNo].downcase  
  ...
  def get_consonants
    ...
    5.times do  #Iterate 5 times
      ...
      if list.include?(input.upcase) == true then
        Console_Screen.cls       #Clear the display area 
        puts "You have guessed the letters #{list.inspect.gsub(/\"/, "")}"
    ...
    3.times do |i|  #i equals 0 on the first iteration of the loop
      ...
      if reply == shortWord then  #The player's guessed the secret word
        ...
        if i == 1 then 
          ...
          print "The word was " + shortWord.sub(/^./,/(?<letter>[^.])/.match(shortWord.upcase)[:letter])  + ". Press Enter to continue."
        else
...
loop do
  ...
  puts "Press h if you would like to see the hidden instructions"
...
if answer == "n" or answer == "N" #See if the player elected not
  ...
  loop do  #Loop forever
    ...
    puts "Press h to view the hidden instructions"

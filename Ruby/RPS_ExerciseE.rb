...
class Game
  ...
  def analyze_results(player, computer)
    ...
    if player == "ROCK" || player == "R" then
      return "Rock crushes scissors. " + winMessage if computer == "SCISSORS" 
      return "Rock equals Rock. " + tieMessage if computer == "ROCK"     
      return "Rock is covered by Paper. " + loseMessage if computer == "PAPER" 
    end
  
    #Analyze the results of the game when the player selects PAPER
    if player == "PAPER" || player == "P" then
      return "Paper covers Rock. " + winMessage if computer == "ROCK" 
      return "Paper equals Paper. " + tieMessage if computer == "PAPER"     
      return "Paper is cut by Scissors. " + loseMessage if computer == "SCISSORS" 
    end
  
    #Analyze the results of the game when the player selects SCISSORS
    if player == "SCISSORS" || player == "S" then
      return "Scissors cut Paper. " + winMessage if computer == "PAPER" 
      return "Scissors equal Scissors. " + tieMessage if computer == "SCISSORS" 
      return "Scissors are crushed by Rock. " + loseMessage if computer == "ROCK" 
    end
  ...
  def display_results(player, computer, result)
    ...
    puts "\n\n\t\t\t================================"
    puts "\a"
    puts "Number of times played: #{$gameCount}."
    puts "Games won: #{$wins}"
    puts "Games tied: #{$ties}"
    puts "Games lost: #{$lost}"
    print "Press Enter to continue. "
    Console_Screen.pause       #Pause the game
